use moon_common::Id;
use moon_config::{BunConfig, DependencyScope, NodeVersionFormat, TypeScriptConfig};
...
const LOG_TARGET: &str = "moon:bun-platform:sync-project";
...
pub async fn sync_project(
    ...
    for (dep_id, dep_cfg) in &project.dependencies {
        ...
        if bun_config.sync_project_workspace_dependencies && !project.is_root_level() {
            let format = &bun_config.dependency_version_format;
