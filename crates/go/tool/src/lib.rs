...
use moon_config::{GoConfig, NodePackageManager};
...
impl GoTool {
    pub fn new(proto: &Proto, config: &GoConfig, version: &str) -> Result<NodeTool, ToolError> {
        let mut node = GoTool {
...
impl Tool for GoTool {
    ...
    async fn setup(
        ...
        if self.tool.is_setup(&version).await? {
            debug!(target: self.tool.get_log_target(), "Go.js has already been setup");
        } else {
