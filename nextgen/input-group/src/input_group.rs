use crate::input_group_error::InputGroupError;
...
impl InputGroup {
    pub fn new<T>(id: T) -> miette::Result<InputGroup>
    where
        T: AsRef<str>,
    {
        let id = Id::new(id)?;

        trace!(id = id.as_str(), "Creating input group");
    ...
    pub fn new_with_source<T, I>(id: T, patterns: I) -> miette::Result<InputGroup>
    where
        T: AsRef<str>,
    ...
    pub fn add(&mut self, input: &InputPath, project_source: &str) -> miette::Result<()> {
        match input {
            InputPath::EnvVar(var) => {
                self.env.push(format!("${var}"));
            }
            InputPath::TokenFunc(_) | InputPath::TokenVar(_) => {
                return Err(InputGroupError::NoTokens(self.id.clone()).into());
            }
            _ => {
    ...
    /// Return the input group as an expanded list of directory paths.
    ...
    /// Return the input group as a list of environment variable names.
    pub fn env(&self) -> miette::Result<&Vec<String>> {
        Ok(&self.env)
    }

    /// Return the input group as an expanded list of file paths.
    ...
    /// Return the input group as a list of file globs (as-is),
    /// relative to the project root.
    pub fn globs(&self) -> miette::Result<&Vec<WorkspaceRelativePathBuf>> {
        if self.globs.is_empty() {
            return Err(InputGroupError::MissingGlobs(self.id.clone()).into());
        }

        Ok(&self.globs)
    }

    /// Return the input group reduced down to the lowest common directory.
