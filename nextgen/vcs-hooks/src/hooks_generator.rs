...
impl<'app> HooksGenerator<'app> {
    ...
    fn create_hooks(&self) -> miette::Result<FxHashMap<&'app String, PathBuf>> {
        ...
        for (hook, commands) in &self.config.hooks {
            let hook_path =
                self.internal_dir
            ...
            debug!(file = ?hook_path, "Creating {} hook", color::file(hook));

            self.create_hook_file(&hook_path, commands)?;

            hooks.insert(hook, hook_path);
        }
    ...
    fn sync_to_vcs(&self, hooks: FxHashMap<&'app String, PathBuf>) -> miette::Result<()> {
        for (hook_name, internal_path) in hooks {
            ...
            if external_path.exists() {
                ...
                self.create_hook_file(&external_path, &[external_command])?;
            }
        }
    ...
    fn create_hook_file(&self, file_path: &Path, commands: &[String]) -> miette::Result<()> {
