...
| Outputs not restored after cache hit                            | `outputs` misconfigured; or <sup>v2.5+</sup> the daemon hydrates in the background and its failure only shows in the daemon logs                                         | Check `.moon/cache/outputs/`; `moon daemon logs`                            | `references/cache-issues.md`    |
...
- **Daemon archiving/hydration failures are invisible in the main process** <sup>v2.5+</sup> — with
  the daemon enabled, task outputs are archived and hydrated in the background, and failures only
  appear in `moon daemon logs`. To rule the daemon out, re-run with `MOON_DAEMON=false`.
