...
For example, scope tasks to Node.js projects with `.moon/tasks/node.yml`, Rust applications with
`.moon/tasks/rust-application.yml`, Go libraries with `.moon/tasks/go-library.yml`, Ruby scripts
with `.moon/tasks/ruby-tool.yml`, so on and so forth!
