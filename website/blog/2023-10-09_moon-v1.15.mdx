...
In this release, we've taken the new step in modernizing our action pipeline, by rewriting the
dependency graph.
...
### Automatic peer dependencies (breaking)
...
on the other project) will now automatically mark that other project as a "peer" dependency. For
example, "b" becomes a peer dependency for "a".
