...
setting called [`typescript.typesRoot`](/docs/config/toolchain#typesroot), that denotes the root
directory for TypeScript related functionality (relative from moon's workspace root).

```yaml title=".moon/toolchain.yml"
typescript:
  typesRoot: './typescript'
```
