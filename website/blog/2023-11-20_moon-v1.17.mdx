...
JavaScript root are different? Well, you were out of luck, until now! We've updated the
[`typescript.rootConfigFileName`](/docs/config/toolchain#rootconfigfilename) and
[`typescript.rootOptionsConfigFileName`](/docs/config/toolchain#rootoptionsconfigfilename) settings
to support directory prefixes.

When we detect a directory prefix, we'll use that as the root directory for TypeScript paths. For
example, let's organize all TypeScript related functionality and configs in a `typescript` folder.

```yaml title=".moon/toolchain.yml"
typescript:
  rootConfigFileName: 'typescript/tsconfig.json'
  rootOptionsConfigFileName: 'typescript/tsconfig.options.json'
```
