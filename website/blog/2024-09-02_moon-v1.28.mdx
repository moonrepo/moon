...
which configures a collection of task options. Presets are a much better pattern, as it allows us to
support multiple presets, and uniquely name our presets. For example, old `local` tasks can be
rewritten as such:
...
And lastly, the `local` setting is now deprecated, and will be removed in v2.
