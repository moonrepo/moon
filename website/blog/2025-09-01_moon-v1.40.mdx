...
Migrating from the legacy toolchains to these new modern WASM toolchains is straightforward. Follow
these steps:

- Move the `node.npm`, `node.pnpm`, and `node.yarn` configuration to its own top-level `unstable_`
  toolchain.
- Move the `version` setting to an `unstable_bun` or `unstable_node` toolchain. If not using
  `version`, set an empty object.
