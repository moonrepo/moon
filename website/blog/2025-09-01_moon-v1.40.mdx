...
1. Because the old `node`/`bun` toolchains are now spread across multiple new toolchains, the task
   [`toolchain` setting](/docs/config/project#toolchain) no longer overrides, and instead merges.
   Otherwise tasks would fail to inherit the necessary functionality.
2. Additionally, task inheritance _may_ not function the same, based on what toolchains are now
   ...
   utilizing `moon project` and `moon task` commands.
