...
> The `moonx` binary is only available when moon is installed via the official installers, and is
> not available through the `@moonrepo/cli` npm package.
...
Additionally, `*.local` files will only be loaded locally, and not in CI environments.
