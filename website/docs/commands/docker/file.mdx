...
- `--image` - Base Docker image to use. Defaults to `scratch` / derived from the toolchain (for
  example, Node.js is `node:latest`), or prompts in the terminal.
...
Options may be customized with the [`docker.file`](../../config/project#file) setting in
