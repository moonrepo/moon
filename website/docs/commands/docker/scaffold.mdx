...
Options may be customized with the `docker.scaffold` setting in
[`moon.yml`](../../config/project#scaffold) for each project, or
[`.moon/workspace`](../../config/workspace#scaffold) for the entire workspace.
