...
- [`docker.scaffold`](../../config/workspace#scaffold) in `.moon/workspace.*` (entire workspace)
- [`docker.scaffold`](../../config/project#scaffold) in `moon.*` (per project)
...
moon][graph]. Otherwise, moon will fail to copy all required files and builds may fail.
