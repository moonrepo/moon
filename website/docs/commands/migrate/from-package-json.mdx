...
- When running a script within another script, the full invocation of `npm run ...`, `pnpm run ...`,
  or `yarn run ...` must be used (including `test`). Shorthand variants are **not** supported, for
  example, `yarn lint` or `pnpm format`. We cannot guarantee that moon will parse these correctly
  otherwise.
...
- Scripts that change directory (`cd ...`), use pipes (`|`), redirects (`>`), or the OR operator
  ...
  as tasks represent that state of a single command execution, including its inputs and outputs.

  However, you can wrap this functionality in a
  [custom script that executes it on the task's behalf](../../faq#how-to-pipe-or-redirect-tasks).

- [Life cycle scripts](https://docs.npmjs.com/cli/v8/using-npm/scripts#life-cycle-scripts) are not
  converted to tasks and will remain in `package.json` since they're required by npm (and other
  package managers). However, their commands _will_ be updated to execute moon commands when
  applicable.
