...
- `toolchains` - All toolchains the task ran in, sorted and comma separated, like
  `node,npm,typescript`. This is the expanded list, so it includes toolchains that were required by
  the configured ones. It's a joined string rather than an array value, as metric backends don't
  handle array attributes consistently. To aggregate by an individual toolchain, match against it
