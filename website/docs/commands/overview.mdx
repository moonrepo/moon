...
Global options **must** be passed _after_ the `moon` binary and _before_ the command.
...
maximum amount of threads to utilize in our thread pool, as dictated by the Tokio runtime. When
changing this value, it _does not_ change how many tasks run in parallel, but _does_ change how many
cores will be utilized by the operating system.

```shell
$ moon --concurrency 1 run app:build
# Or
