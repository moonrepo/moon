...
Alongside traces, moon records metrics for every action and operation that runs within the pipeline.
Actions are the units of work in the action graph (running a task, syncing a project, installing
dependencies, and so on), while operations are the individual steps _within_ an action (generating a
hash, hydrating outputs from the cache, executing the command, and so on).
...
Action metrics are recorded with the following attributes, allowing them to be grouped and filtered
by what the action operated on.
...
Since the `target` attribute is recorded for every task that runs, the cardinality of
`moon.action.*` metrics scales with the number of tasks in your workspace. If this is a problem for
your observability backend, drop or aggregate away the attribute within your OTEL collector.
