...
- If the current branch is the [`vcs.defaultBranch`](../config/workspace#defaultbranch), then
  compare against the previous revision of the default branch (`HEAD~1`).
- If `--upstream` is provided, then compare the defined base (`--base`) against head (`--head`).
- Otherwise, touched files are based on your local index (`git status`).
...
$ moon query touched-files --upstream --base <branch> --head <commit>
```
...
- `--upstream` - Compare against upstream instead of using local state.
