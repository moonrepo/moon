...
- Dependencies between projects (`dependsOn`) and tasks (`deps`). Task dependency contribution can
  be tuned per-dependency via
  [`cacheStrategy`](../config/project#cache-strategy).
- **For Deno tasks**:
  - Deno version.
...
> cache. Enable it via the
> [`experiments.casOutputsCache`](../config/workspace#casoutputscache) setting.
