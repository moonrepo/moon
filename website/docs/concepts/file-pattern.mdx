...
When configuring [`inputGroups`](../config/project#inputgroups),
[`inputs`](../config/project#inputs), and [`outputs`](../config/project#outputs), all listed file
paths and globs are relative from the project root they will be ran in. They _must not_ start with
`./`, or traverse upwards with `..`.
...
When configuring [`inputGroups`](../config/project#inputgroups),
[`inputs`](../config/project#inputs), and [`outputs`](../config/project#outputs), a listed file path
or glob can be prefixed with `/` to resolve relative from the workspace root, and _not_ the project
root.
