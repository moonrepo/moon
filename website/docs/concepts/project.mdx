...
configured within [`.moon/workspace.*`](../config/workspace), as a key within the
...
- **Explicit dependencies** - These are dependencies that are explicitly defined in a project's
  [`moon.*`](../config/project) config file, using the [`dependsOn`](../config/project#dependson)
  setting.
...
Projects can be configured with an optional [`moon.*`](../config/project) in the project root, or
through the optional workspace-level [`.moon/tasks/**/*`](../config/tasks).
