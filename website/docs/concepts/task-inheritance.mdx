...
Workspace-level tasks (also known as global tasks) are defined in [`.moon/tasks/**/*`][tasks], and
...
Project-level [`env`](../config/project#env) and [`fileGroups`](../config/project#filegroups) are
also merged with inherited workspace-level values using these strategies. Unlike tasks, the strategy
is configured per project with the
[`workspace.mergeStrategies`](../config/project#mergestrategies)
setting. <VersionLabel version="2.5.0" />
