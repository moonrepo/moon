...
Top-level tasks (also known as global tasks) are defined in [`.moon/tasks.yml`](../config/tasks),
and are inherited by default. However, projects are able to include, exclude, or rename inherited
tasks using the [`workspace.inheritedTasks`](../config/project#inheritedtasks) in
