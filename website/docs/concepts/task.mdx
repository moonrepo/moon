...
- **Run** - Task runs a one-off, long-running, or never-ending process, and is derived from the
  [`options.persistent`](../config/project#persistent) setting.
...
output is not captured, and the task is marked as [persistent](#persistent).

To mark a task as local only, enable the [`preset`](../config/project#preset) setting.
...
displayed in a project's tasks list, but can be inspected with [`moon task`](../commands/task).
...
However in moon, this is a non-issue, as persistent tasks are handled uniquely in the pipeline. They
are ran when they are processed in the action graph, like any other task, but they never block the
actions that come after them. This is perfect for a few reasons:
...
- Persistent tasks run alongside other tasks, instead of waiting for the entire graph to finish.
- Dependencies of persistent tasks are guaranteed to have ran and completed.

Furthermore, a non-persistent task _cannot_ depend on a persistent task, as it would never run, and
we error during task building when this happens.

To mark a task as persistent, enable the [`options.persistent`](../config/project#persistent)
setting.
...
Tasks can be configured per project through [`moon.*`](../config/project), or for many projects
through [`.moon/tasks/**/*`](../config/tasks).
