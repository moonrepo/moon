...
libraries", so on an so forth.

When resolving configuration files, moon will locate and merge files in the following order, from
...
support a repository that is comprised of any combination of these 3. As such, JavaScript (and
TypeScript) based projects have a special lookup using [`platform`](../config/project#platform)
