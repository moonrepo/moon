...
task. They provide a way of accessing input group paths, referencing values from other task fields,
...
These functions reference input groups by name, where the name is passed as the argument.

### `@group`

> Usable in `args`, `env`, `inputs`, and `outputs`.

The `@group(input_group)` token is a standard token that will be replaced with the input group items
...
The `@dirs(input_group)` token will be replaced with an expanded list of directory paths, derived
from the input group of the same name. If a glob pattern is detected within the input group, it will
...
The `@files(input_group)` token will be replaced with an expanded list of file paths, derived from
the input group of the same name. If a glob pattern is detected within the input group, it will
...
The `@globs(input_group)` token will be replaced with the list of glob patterns as-is, derived from
the input group of the same name. If a non-glob pattern is detected within the input group, it will
be ignored.
...
The `@root(input_group)` token will be replaced with the lowest common directory, derived from the
input group of the same name. If a glob pattern is detected within the input group, it will walk the
