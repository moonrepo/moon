...
> Usable in `command`, `script`, `args`, and `env` only.
...
The top-level fields (like `name` and `description`) will be used as-is. If the setting is not
defined, it will default to nothing or an empty string. For lists of values, they will be joined
with `,`.
...
- `$projectType` - The type of project, as defined with [`type`](../config/project#type).
...
- `$taskPlatform` - The platform that task will run against, as defined in
  [`moon.yml`](../config/project#platform-1).
