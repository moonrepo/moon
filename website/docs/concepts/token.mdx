...
[`args`](../config/project#args), [`inputs`](../config/project#inputs), and
[`outputs`](../config/project#outputs) when configuring a task. They provide a way of accessing file
group paths, referencing values from other task fields, and referencing metadata about the project
and task itself.
...
> Usable in `command`, `args`, and `inputs` only.
