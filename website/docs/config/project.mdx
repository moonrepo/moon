...
- `optional` (`boolean`) - When hashing and set to `true` and the file is missing, will not log a
  warning. When set to `false` and the file is missing, will fail with an error. When not defined,
...
- `cache` (`boolean`) - When gathering inputs for hashing, determines whether the globs results
  should be cached for the duration of the moon process. Defaults to `true`.
