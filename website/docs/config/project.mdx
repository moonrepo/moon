...
- `*` - A custom language for use in [task inheritance](./tasks). Values will be converted to
  kebab-case. <VersionLabel version="0.25" />
...
## `platform`<VersionLabel version="0.24" />
...
The [strategy](../concepts/task#merge-strategies) to use when merging the [`env`](#env-1) map with
an inherited task. Defaults to "append".
...
### `typescript`<VersionLabel version="0.25" />
