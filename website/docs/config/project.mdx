...
for a project, override inherited tasks, and more. When used, this file must exist in the project
root, as configured in [`projects`](./workspace#projects).
...
patterns or literal file paths. These groups are then used by [tasks](#tasks) to calculate
functionality like cache computation, affected files since last change, deterministic builds, and
more. By default, this setting _is not required_ for the following reasons:
...
[globally](./global-project)). This enables enforcement of organizational patterns across all
...
wrap an npm or shell command. This setting requires a map, where the key is a unique identifier for
the task, and the value is an object of task parameters.
...
All parameters within a task are optional, except for `command` which is required _unless_ you are
inheriting a global task of the same name.

### `command`<Label text="Required" />

> `string`

The `command` field is the name of an npm binary or native shell command to run when executing the
...
By default a task assumes the command is an npm binary, and if you'd like to reference a shell
command, you'll also need to set the [`type`](#type) to "shell".
...
The `args` field is a list of arguments to pass on the command line when executing the task.
...
The `deps` field is a list of other tasks (known as [targets](../concepts/task#targets)), either
within this project or found in another project, that will be executed _before_ this task. It
achieves this by generating a directed task graph based on the project graph.
...
task based on files that have been touched since the last time the task has been ran. If no touched
files align with the inputs, the task will complete instantly.
...
(should rarely be used), prefix the path with a "/".
...
to execute it with. Accepts "node" or "shell" and defaults to "node".
