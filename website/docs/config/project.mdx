...
- `outputs` - Use the dependency task's outputs instead of its hash for cache invalidation. The
  current task is only invalidated when the dependency's outputs change, not when its inputs
  change. Useful for build tasks where you only care about a dependency's outputs, not what
