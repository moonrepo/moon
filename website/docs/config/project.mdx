...
<HeadingApiLink to="/api/types/interface/ProjectConfig#inputGroups" />

Defines [input groups](../concepts/input-group) to be used by local tasks. By default, this setting
_is not required_ for the following reasons:

- Input groups are an optional feature, and are designed for advanced use cases.
- Input groups defined in [`.moon/tasks.yml`](./tasks) will be inherited by all projects.

When defined this setting requires a map, where the key is the input group name, and the value is a
...
> The code snippet above is merely an example of input groups. Feel free to use those groups as-is,
...
Whether to run the command within a shell or not. Defaults to `true` for system platform or Windows,
and `false` otherwise. The shell to run is determined by the [`unixShell`](#unixshell) and
...
`sh`, or `zsh`. If not defined, will derive the shell from the `SHELL` environment variable, or
defaults to `sh`.
...
Git) or `pwsh`. If not defined, defaults to `pwsh`.
