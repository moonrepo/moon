...
> To apply this setting to primary targets as well, enable the
> [`experiments.explicitTaskOutputStyle`](./workspace#explicittaskoutputstyle) setting. To override
> this setting for all tasks in a single run, pass
> [`--output-style`](../commands/exec#options) on the command line.
...
are handled differently than non-persistent tasks in the action graph. When running a target,
persistent tasks are ran _in parallel_ with other tasks, after all their dependencies have
completed, and they never block the tasks that come after them.
...
[Persistent dependencies](../concepts/task#persistent) never complete, so they are skipped when
ordering the dependencies that follow them. Those dependencies are instead ordered against the
previous dependency that does complete, while the persistent one starts when it is reached.
