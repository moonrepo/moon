...
touched files, typically through the [`moon ci`](../commands/ci) and [`moon run`](../commands/run)
commands. Defaults to `true` unless the [`local`](#local) setting is disabled, but is _always_ true
when a task defines [`outputs`](#outputs).
...
`ion`, `nu`, `pwsh`, `xonsh`, or `zsh`. If not defined, will derive the shell from the `SHELL`
environment variable, or defaults to `sh`.
...
Git), `elvish`, `fish`, `nu`, `pwsh`, or `xonsh`. If not defined, defaults to `pwsh`.
