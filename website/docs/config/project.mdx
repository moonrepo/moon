...
The `moon.yml` configuration file (formerly `project.yml`) _is not required_ but can be used to
define additional metadata for a project, override inherited tasks, and more at the project-level.
When used, this file must exist in a project's root, as configured in
...
- `bash` - A [Bash](<https://en.wikipedia.org/wiki/Bash_(Unix_shell)>) based project (Nix only).
