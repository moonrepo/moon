...
When extending another task, the same [merge strategies](../concepts/task-inheritance#merge-strategies) used for
...
[project relative file patterns](../concepts/file-pattern#project-relative)) that are _created_ as a
result of executing this task, typically from a build or compilation related task. Outputs are
necessary for [incremental caching and hydration](../concepts/cache). If you'd prefer to avoid that
...
The [strategy](../concepts/task-inheritance#merge-strategies) to use when merging the [`args`](#args) list with
...
The [strategy](../concepts/task-inheritance#merge-strategies) to use when merging the [`deps`](#deps) list with
an inherited task. Defaults to "append".
...
The [strategy](../concepts/task-inheritance#merge-strategies) to use when merging the [`env`](#env-1) map with
an inherited task. Defaults to "append".
...
The [strategy](../concepts/task-inheritance#merge-strategies) to use when merging the [`inputs`](#inputs) list
...
The [strategy](../concepts/task-inheritance#merge-strategies) to use when merging the [`outputs`](#outputs) list
with an inherited task. Defaults to "append".
...
Marks the task as persistent (continuously running). Persistent tasks are a special case that are
