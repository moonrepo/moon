...
command, you'll also need to set the [`type`](#type-1) to "system". We do our best to automatically
detect this, but it's not accurate in all scenarios.
...
Dependencies (`node_modules`) ignore this setting and will continue to install at the workspace root
using the workspace configured Node.js version and package manager.
