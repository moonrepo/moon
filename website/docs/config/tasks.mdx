...
The `.moon/tasks.yml` file configures input groups and tasks that are inherited by _every_ project
in the workspace, while `.moon/tasks/**/*.yml` configures for projects based on their language or
type. [Learn more about task inheritance!](../concepts/task-inheritance)
...
For map-based settings, `inputGroups` and `tasks`, entries from both the extended configuration and
...
<HeadingApiLink to="/api/types/interface/InheritedTasksConfig#inputGroups" />

> For more information on input group configuration, refer to the
> [`inputGroups`](./project#inputgroups) section in the [`moon.yml`](./project) doc.

Defines [input groups](../concepts/input-group) that will be inherited by projects, and also enables
...
File paths and globs used within a input group are relative from the inherited project's root, and
...
Implicit dependencies are _always_ inherited, regardless of the [`mergeDeps`](#mergedeps) option.
...
Implicit inputs are _always_ inherited, regardless of the [`mergeInputs`](#mergeinputs) option.
