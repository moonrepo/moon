...
Projects can override or merge with these settings within their respective [`moon.*`](./project).
...
For map-based settings, `env`, `fileGroups`, and `tasks`, entries from both the extended
...
## `env`<VersionLabel version="2.5.0" />

<HeadingApiLink to="/api/types/interface/InheritedTasksConfig#env" />

Defines a map of environment variables that will be inherited by all matching projects, and are
merged into each project's [`env`](./project#env) setting, with project-level variables taking
precedence on conflicting keys. The merge behavior can be customized per project with the
[`workspace.mergeStrategies.env`](./project#mergestrategies) setting.
...
> [`fileGroups`](./project#filegroups) section in the [`moon.*`](./project) doc.
...
As of v2.4, task options can also be defined per project with the
[`taskOptions`](./project#taskoptions) setting in [`moon.*`](./project), which sits later in the
inheritance chain and takes precedence over these workspace-level defaults.
