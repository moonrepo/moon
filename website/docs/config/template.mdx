...
The type of value for the variable. Accepts `boolean`, `string`, `object`, `number`, or `enum`.
Floats _are not supported_, use strings instead.
...
Your basic primitives: boolean, numbers, strings, and collections: objects.

<Tabs
  groupId="types"
  defaultValue="boolean"
  values={[{ value: 'boolean' }, { value: 'number' }, { value: 'object' }, { value: 'string' }]}
>
...
Marks the variable as required during _prompting only_. For strings and objects, will error for
empty values (`''`). For numbers, will error for zero's (`0`).
