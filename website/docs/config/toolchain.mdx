...
to the [`rootConfigFileName`](#rootconfigfilename) setting. Defaults to `false`.
...
setting when syncing projects as references. Defaults to `tsconfig.json`, and supports nesting in
directories.
...
Defaults to `tsconfig.options.json`, and supports nesting in directories. This setting is used in
...
`paths` compiler option would be updated to the following when syncing. The index file may exist in
a `src` folder, or the root of the package.
