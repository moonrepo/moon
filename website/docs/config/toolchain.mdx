...
## `unstable_nub`<VersionLabel version="2.5.0" />

Run `moon toolchain info unstable_nub` for all available settings.

:::info

This toolchain requires the [`javascript`](#javascript) toolchain to also be enabled. Unlike other
package managers, it does _not_ require the [`node`](#node) toolchain, as
[nub](https://nubjs.com) is a standalone binary.
...
Run `moon toolchain info unstable_poetry` for all available settings.
...
Run `moon toolchain info unstable_ruby` for all available settings.
