...
Defines the explicit Rust version/channel to use. We require an explicit and semantic major, minor,
and patch version, to ensure the same environment is used across every machine.
...
This is a special setting that ensure other Rust/Cargo processes outside of our toolchain are
