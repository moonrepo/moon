...
Globs can match both folders and files. When a folder is matched, it's used as the project root
as-is, while a matched file will use its parent folder as the project root. Matching files is
useful for only including folders that contain a specific file, like a moon configuration file, or
a toolchain specific manifest.
...
[`id`](./project#id) setting in [`moon.*`](./project). Furthermore, globbing **does risk the chance
...
Matching any kind of file is a new feature in moon v2.5. In previous versions, only moon
configuration files (`moon.*`) could be matched, while all other files were skipped. Additionally,
for the workspace root itself, only a matched moon configuration file will create a
[root-level project](../guides/root-project), all other root files are ignored.
...
it inherits the folder name, but this has a high chance of collision. Instead you can configure
...
## `cache`<VersionLabel version="2.3.0" />

<HeadingApiLink to="/api/types/interface/WorkspaceConfig#cache" />

Configures aspects of the caching engine and layer. These settings primarily tune the
content-addressable storage (CAS) cache and the native file hasher.

### `cas`

<HeadingApiLink to="/api/types/interface/CacheConfig#cas" />

Configures aspects of the content-addressable storage (CAS) cache.

#### `maxSize`<VersionLabel version="2.4.0" />

<HeadingApiLink to="/api/types/interface/CacheCasConfig#maxSize" />

Maximum total size of the local CAS cache, for example `10gb` or `512mib`. When the limit is
exceeded, the least recently used task outputs are evicted. Unlimited when not set.
...
Can also be set with the `MOON_CACHE_CAS_MAX_SIZE` environment variable.

#### `verifyIntegrity`

<HeadingApiLink to="/api/types/interface/CacheCasConfig#verifyIntegrity" />

Re-verifies the hash of cached content on every read. When enabled, reads are slower but on-disk
corruption is detected. Defaults to `false`.
...
Can also be enabled with the `MOON_CACHE_CAS_VERIFY_INTEGRITY` environment variable.

### `unstable_sharedWorktreeCache`<VersionLabel version="2.5.0" />

<HeadingApiLink to="/api/types/interface/CacheConfig#sharedWorktreeCache" />

Shares the content-addressable storage (CAS) cache between all VCS worktrees on the same machine,
instead of each worktree maintaining its own isolated cache. Only blobs and manifests are shared,
as they are portable, while hashes, locks, states, and more remain worktree-specific.

The shared cache is stored in the base checkout's `.moon/cache` directory, or in
`~/.moon/cache/shared` when the repository root has no checkout (bare clones).
...
Can also be enabled with the `MOON_CACHE_SHARED_WORKTREE_CACHE` environment variable.

:::info

This setting requires the [`casOutputsCache`](#casoutputscache) experiment to be enabled.
...
On the project side, we would configure [`moon.*`](./project#tags) like so:
...
## `unstable_daemon`<VersionLabel version="2.2.0" />

<HeadingApiLink to="/api/types/interface/WorkspaceConfig#daemon" />

Enables the background [daemon](../guides/daemon), a server process that keeps the workspace graph
hot in memory, watches for file changes, and processes heavy operations (like task output archiving
and hydration) in the background. Defaults to `false`.

```yaml title=".moon/workspace.yml"
unstable_daemon: true
```

Can also be enabled with the `MOON_DAEMON` environment variable.

:::caution

The daemon is currently **unstable**, and its behavior and configuration may change in future
releases. Learn more about it in the [official guide](../guides/daemon).
...
### `asyncAffectedTracking`<VersionLabel version="2.2.0" />
...
### `asyncGraphBuilding`<VersionLabel version="2.2.0" />
...
### `casOutputsCache`<VersionLabel version="2.3.0" />

<HeadingApiLink to="/api/types/interface/ExperimentsConfig#casOutputsCache" />

Stores task outputs in a local content-addressable storage (CAS) cache, instead of the legacy
tarball-based local cache. This shares the same content-addressed format used by the remote cache,
enabling deduplicated storage across tasks and faster hydration. Defaults to `false`.
...
Can also be enabled with the `MOON_EXPERIMENT_CAS_OUTPUTS_CACHE` environment variable.

> The CAS layer can be tuned through the top-level [`cache`](#cache) setting.

### `explicitTaskOutputStyle`<VersionLabel version="2.6.0" />

<HeadingApiLink to="/api/types/interface/ExperimentsConfig#explicitTaskOutputStyle" />

Applies a task's [`outputStyle`](./project#outputstyle) option to _all_ targets, instead of only
transitive (non-primary) ones. By default, a primary target (one you explicitly requested on the
command line) always displays its output and ignores the configured style. Defaults to `false`.

```yaml title=".moon/workspace.yml" {2}
experiments:
  explicitTaskOutputStyle: true
```

Can also be enabled with the `MOON_EXPERIMENT_EXPLICIT_TASK_OUTPUT_STYLE` environment variable.

### `nativeFileHashing`<VersionLabel version="2.3.0" />

<HeadingApiLink to="/api/types/interface/ExperimentsConfig#nativeFileHashing" />
...
Can also be enabled with the `MOON_EXPERIMENT_NATIVE_FILE_HASHING` environment variable.
...
<HeadingApiLink to="/api/types/interface/NotifierConfig#webhookAcknowledge" />
