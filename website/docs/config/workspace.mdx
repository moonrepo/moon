...
Configures constraints between projects that are enforced during
[project syncing](../commands/sync). This is also known as project boundaries.
...
- Applications can _only_ depend on libraries.
...
For example, let's enforce that Next.js projects using the `next` tag can only depend on projects
that support React using the `react` tag.
...
> Projects with no tags are considered an allowed relationship for easier interoperability.
