...
> Unknown project types are ignored during enforcement.
...
that support React using the `react` tag. If a dependency does not have one of the configured
allowed tags, in this case `react`, an error will occur.
