...
- Script names will be converted to kebab-case.
- Tasks defined in `.moon/project.yml` or `project.yml` take precedence over scripts of the same
  name.

To verify inferred tasks and their options, run [`moon project <id> --json`](../commands/project)!
...
The functionality that powers this feature, is the same as
[`moon migrate from-package-json`](../commands/migrate/from-package-json), so all
[requirements and caveats](../commands/migrate/from-package-json#caveats) also apply here! Be aware
of these when utilizing this feature, especially in regards to `runInCI`!
