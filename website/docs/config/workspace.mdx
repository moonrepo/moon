...
Configures code owners integration across the entire workspace.
...
[`owners`](./project#owners) in the workspace. The format and location of the `CODEOWNERS` file is
based on the [`vcs.provider`](#provider) setting. Defaults to `false`.
