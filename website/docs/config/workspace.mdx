...
<HeadingApiLink to="/api/types/interface/VcsConfig#syncHooksOnRun" />

Will automatically generate [hook scripts](#hooks) to `.moon/hooks` and sync the scripts to the
local VCS checkout, everytime a [target is run](../concepts/target). The hooks format and location
is based on the [`vcs.manager`](#manager) setting. Defaults to `false`.
