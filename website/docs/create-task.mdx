...
```yaml title="<project>/moon.yml" {2,3}
tasks:
	build:
...
```yaml title="<project>/moon.yml" {4-7}
tasks:
	build:
		command: 'webpack build --mode production --no-stats'
		inputs:
...
```yaml title="<project>/moon.yml" {3,8,9}
tasks:
	build:
