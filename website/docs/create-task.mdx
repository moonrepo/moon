...
[`moon.*`](./config/project), or inherited by many projects with
[`.moon/tasks/**/*`](./config/tasks), but can also be inferred from a language's ecosystem.
...
a project using [`moon.*`](./config/project).
