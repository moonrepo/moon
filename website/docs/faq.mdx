...
- A change to every project relative file will trigger the task as affected, even those not
  necessary for the task, as `inputs` default to `**/*`. Granular input control is lost.
- A change to workspace relative files _will not_ trigger the task as affected. For example, a
  change to `/prettier.config.js` would not be detected for a `npm run format` script.
