...
to keep using `package.json` scripts, enable the
[`node.inferTasksFromScripts`](./config/workspace#infertasksfromscripts) setting.

View the [official documentation](./migrate-to-moon#using-packagejson-scripts) for more information
on this approach, including risks, disadvantages, and caveats.
