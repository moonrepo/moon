...
- Have `inputPaths`, `inputGlobs`, and `inputVars` expanded correctly from [`inputs`][inputs]?
- Have `outputPaths` and `outputGlobs` expanded correctly from [`outputs`][outputs]?
