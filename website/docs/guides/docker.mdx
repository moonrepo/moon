...
worries, moon can be utilized with Docker!
...
- Running `moon docker` commands _within_ the `Dockerfile`, using staged builds.
...
This can all be achieved by the [`moon docker scaffold`][command] command, which scaffolds a
...
This assumes [`moon docker scaffold <project>`][command] is ran outside of the `Dockerfile`.
...
Luckily our [`moon docker scaffold <project>`][command] command has already done this for us! Let's
...
[command]: ../commands/docker/scaffold
