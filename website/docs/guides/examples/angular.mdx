...
[ESLint plugin](https://github.com/angular-eslint/angular-eslint), which is great, but complicates
...
- Use Angular's ESLint plugin solution only.
...
[`@angular-eslint`](https://nextjs.org/docs/basic-features/eslint#eslint-config) plugin need in the
...
Since Angular has some specific rules, we'll need to tell the ESLint plugin to overrides the default
ones. This can be achieved with a project-level `.eslintrc.json` file.
...
This configuration uses the `@angular-eslint/builder` for linting and specifies that all TypeScript
files and HTML files within the src directory should be linted. Adding this lint target is crucial
for ensuring that the linting process is properly configured and integrated with Angular's build
system.
