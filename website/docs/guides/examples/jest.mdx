...
tasks:
	test:
		...
		inputs:
			...
			- 'src/**/*'
			- 'tests/**/*'
			# A config in the project, any format
			- 'jest.config.*'
```
...
A root-level Jest config is not required, instead, use a [preset](#sharing) to share configuration.
...
> You can take this a step further by passing the `--preset` option in the
> [task above](#adding-a-task), so that all projects inherit the preset by default.
