...
<HeadingApiLink to="https://github.com/moonrepo/examples/blob/master/.moon/tasks.yml#L22" />
...
- A `buildPackage` task in [`.moon/tasks.yml`](../../config/tasks), which can be inherited by
  package based projects.
- A `build` task in each [`moon.yml`](../../config/project).
