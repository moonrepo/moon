...
After setting up your Storybook, ensure your `.moon/project.yaml` has the following scripts:
```yaml
tasks:
    storybook:
        local: true
...
Alternatively if you only have one Storybook project, you can add the task to your `project/moon.yaml`, to run Storybook you run:
```
...
Storybook 7 uses Vite out of the box, as such no configuration is required but should you choose to extend the Vite config can you do so by passing in `viteFinal`: 
...
If you opt to use Webpack with your Storybook instance, you can do so by passing it in the config:
...
You can use Jest to test your stories, Storybook ships with first party plugins for improved DX
...
Add the following script to your Moon project:
```
"scripts": {
...
Enable the plugins in your `.storybook/main.ts`
```ts
