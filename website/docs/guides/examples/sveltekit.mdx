...
SvelteKit provides an option to setup ESLint along with your project, you can use a [global `lint` task](./eslint).

We encourage using the global `lint` task for consistency across all projects within the repository.
With this approach, the `eslint` command itself will be ran and the `svelte3` rules will still be used.
