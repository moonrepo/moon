...
With moon, this is no longer an issue with the [`rust.cargoBins`](../../config/toolchain#cargobins)
setting. This setting requires a list of crates (with optional versions) to install, and moon will
install them as part of the task runner `InstallRustDeps` action. Furthermore, binaries will be
installed with [`cargo-binstall`](https://crates.io/crates/cargo-binstall) in an effort to reduce
...
For moon, if you're looking to persist target results across CI runs, you can utilize the
[`runner.archivableTargets`](../../config/workspace#archivabletargets) setting. This is useful for
caching `cargo check`, `cargo clippy`, and other Cargo tasks that _do not_ produce outputs.
