...
To demonstrate this, let's configure a `pre-commit` hook that runs a `lint` task for affected
...
All commands are executed globally and must exist on `PATH`, for every machine. If `moon` is
installed locally, you can execute it using a repository relative path, like
...
solution for everyone can be difficult, so with moon, we opted to support both options. Choose the
solution that works best for your project, team, or company!

### Automatically for everyone

If you'd like hooks to be enforced for every contributor of the repository, then simply enable the
[`vcs.syncHooksOnRun`](../config/workspace#synchooksonrun) setting in
...
2. We then sync these generated hook scripts with the current VCS. For Git, we create `.git/hooks`
   files that execute our generated scripts, using repository root relative commands. Any existing
   VCS hooks _will be overwritten_.
