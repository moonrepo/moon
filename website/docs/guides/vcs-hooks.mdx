...
[`.moon/workspace.*`](../config/workspace). This setting requires a map of hook names (in the format
...
[`vcs.sync`](../config/workspace#sync) setting in [`.moon/workspace.*`](../config/workspace). This
will automatically generate hook scripts and link them with the local VCS checkout, everytime a
[task](../concepts/target) is ran.
...
hook commands you define in the [`.moon/workspace.*`](../config/workspace) file.
