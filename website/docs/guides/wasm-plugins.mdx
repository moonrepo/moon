...
To simplify development, we provide built-in macros for the host functions above. Continue reading
...
[`virtual_path!`](https://docs.rs/warpgate_pdk/latest/warpgate_pdk/macro.virtual_path.html) and
[`real_path!`](https://docs.rs/warpgate_pdk/latest/warpgate_pdk/macro.real_path.html) macros can be
used for such situations, which use the `to_virtual_path` and `from_virtual_path` host functions
respectively.
...
The [`host_env!`](https://docs.rs/warpgate_pdk/latest/warpgate_pdk/macro.host_env.html) macro can be
used to read and write environment variables on the host, using the `set_env_var` and `get_env_var`
...
command does not exist on `PATH`, an error is thrown.

This macros supports three modes: pipe, inherit, and raw (returns `Result`). If you want full
control, use the input mode and provide
[`ExecCommandInput`](https://docs.rs/warpgate_pdk/latest/warpgate_pdk/struct.ExecCommandInput.html).
...
is composed of 2 parts separated by `:`, the former is the scope (or protocol), and the latter is
the location.
...
config file). Files will be used as-is, while URLs will be downloaded to `~/.moon/plugins` or
`~/.proto/plugins`.
