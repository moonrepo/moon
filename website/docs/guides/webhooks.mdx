...
This event _does not_ trigger if [`target-output.archiving`](#target-outputarchiving) did not run or
failed to run.
...
This event _does not_ trigger if [`target-output.hydrating`](#target-outputhydrating) did not run or
failed to run.
