...
Like the base run target, but runs the task in a persistent process that never exits. All persistent
tasks are run in parallel as the last batch in the graph.
