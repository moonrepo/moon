...
Although moon only supports the JavaScript ecosystem at this time, our long-term vision is to be a
...
more internal integrations and automations, but requires more work to properly implement. Internally
each tier maps to a Rust crate.
...
The tiers (and crates) mentioned above can be demonstrated using a directed acyclic graph. Below is
an example of how a language is implemented, from tier 0 to tier 3.
