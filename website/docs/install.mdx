...
In a shell that supports Bash, run:

```
curl -fsSL https://moonrepo.dev/install.sh | bash
```

This will install moon to `~/.moon/tools/moon` and create a symlink in `/usr/local/bin`.
...
This will install moon to `~\.moon\tools\moon` and append to the `PATH` environment variable for the
...
We provide no formal mechanism for upgrading the moon binary. However, you can re-run the install
