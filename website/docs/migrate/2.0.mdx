...
- Changed the output of `moon action-graph --json` to a new JSON structure (now matches the project
  and task graphs).
- Changed the destination path for `moon init` to an argument instead of the `--to` option.
- Updated `moon templates` to render a table instead of a list.
...
- Removed the `moon migrate from-turborepo` command. Use the `migrate-turborepo` extension instead.

### Arguments, options, and flags

- Renamed all to kebab-case instead of camelCase (example, `--updateCache` -> `--update-cache`).
