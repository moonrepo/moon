...
| `${VAR:-default}`   | ⛔️ Not supported                                          | Use default value if variable empty                           |
| `${VAR:+alternate}` | ⛔️ Not supported                                          | Use alternate value if variable non-empty                     |
...
higher tier(s). Refer to the following table:
...
If you don't want to inherit a shell variable, you can override it in the task with a null value.
