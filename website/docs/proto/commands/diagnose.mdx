...
If [lockfiles](../config#unstable-lockfile) are in use, proto will also audit their health, and
report warnings for records that are missing a resolved version, duplicate records for the same
specification, and stale records that no longer match a configured version. <VersionLabel version="0.60.0" />
