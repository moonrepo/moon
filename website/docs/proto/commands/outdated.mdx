...
When a [lockfile](../lockfile) is in use, a "Locked" column will also be displayed,
...
configuration. <VersionLabel version="0.60.0" />

Versions defined in [environment specific configurations](../config#environment-mode)
(`.prototools.<env>`) are also updated by `--update`, including the records in their
[sibling lockfiles](../lockfile#environment-scopes). Previously these versions were skipped with a
warning. <VersionLabel version="0.61.0" />
