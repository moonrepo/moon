...
the location, or use the `--tool-native` option to store it in a tool specific location, like the `devEngines` field in the `package.json` for JavaScript tools like Node and npm.
JS projects.
...
- `--tool-native` - Pins the value in the local
