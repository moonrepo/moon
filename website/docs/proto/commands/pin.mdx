...
$ proto pin npm latest --resolve --devEngines.packageManager
```

By default this will update the local [`./.prototools`](../config) file. Pass `--to` to customize
the location, or use the `--devEngines.x` options below to store in the `package.json` for
JS projects.
...
- `--devEngines.runtime` and `--devEngines.packageManager` - Pins the value in the local
  `package.json`'s `devEngines` field, rather than any `.prototools` file. This is exclusively for
  JavaScript tooling (Node, Bun, Deno, npm, pnpm, Yarn, etc).
  <VersionLabel version="0.54.3">
