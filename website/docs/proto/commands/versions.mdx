...
denoted with a timestamp, and if a version has a record in a
[lockfile](../config#unstable-lockfile), it will be denoted with a "locked" label.
...
[version requirement](../tool-spec#requirements-and-ranges) as the second argument. Only versions
