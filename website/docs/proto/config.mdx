...
[Plugins](./plugins) can also be configured on a per-project basis, by defining a `[plugins]`
section, that maps a plugin to a locator string. The map key is the plugin name _in kebab-case_,
which is used as the binary/tool name in proto, and also the name for configuration and cache
purposes. The map value is a locator string that defines a protocol and source location, and
supports the following formats:

- `source:<location>` - A secure URL or a file path relative from the parent configuration.
