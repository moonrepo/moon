...
Each configuration file also owns its own [lockfile](./lockfile#environment-scopes), so
`.prototools.production` is locked to `.protolock.production`, while `.prototools` remains locked
to `.protolock`. <VersionLabel version="0.61.0" />
...
#### `builtin-backends`<VersionLabel version="0.60.0" />

Can be used to customize the [built-in backends](./tool-spec#backends) within proto. Can disable all built-ins by passing `false`, or enabling a select few by name.
Defaults to `true`, which enables all.
...
# Enable some
builtin-backends = ["asdf"]
```

#### `builtin-tools`<VersionLabel version="0.60.0" />

Can be used to customize the [built-in tools](./tools#built-in) within proto. Can disable all
...
> Before v0.60, both of these settings were a single `builtin-plugins` setting, which can still be
> used as an alias for `builtin-tools`.

#### `cache-duration`<VersionLabel version="0.50.1" />

The duration in seconds in which to cache downloaded plugins. Defaults to 180 days (6 months).
...
> Before v0.61, this defaulted to 30 days. It was increased because of GitHub's unreliability, as
> re-downloading a plugin that hasn't changed is a common source of transient install failures.

#### `community-tools`<VersionLabel version="0.62.0" />

Whether to automatically resolve tools from the official
[community plugins](./plugins#community-plugins) registry, when a tool has no configured plugin
locator. Defaults to `true` or `PROTO_COMMUNITY_TOOLS`.
...
> The registry is only consulted after your own `[plugins.tools]`, the `plugin` fields, and the
> [built-in tools](#builtin-tools) have all been exhausted, so a locator you configure yourself
> always wins.
...
Since v0.60, a lockfile is scoped to the configuration file in which this setting was enabled:
...
- Versions defined in nested configuration files are not included in a parent's lockfile. To lock a
  nested configuration, enable this setting in that file as well.

Since v0.61, [environment specific configurations](#environment-mode) own their own lockfile. When
`PROTO_ENV` is set, a `.prototools.<env>` file is locked to a sibling `.protolock.<env>` file,
instead of sharing the directory's `.protolock`. Environment configurations inherit this setting
from the `.prototools` file in the same directory, but can override it.

> Refer to the [lockfiles documentation](./lockfile) for more information on how lockfiles work.
...
- `auth` (bool) - Whether the registry requires authentication. When true, proto will attempt to
  load credentials for this host from the Docker config. <VersionLabel version="0.57.0" />
- `default` (bool) - Whether this registry should be used as the default when the locator only
  provides an identifier (no registry host or namespace). <VersionLabel version="0.57.0" />
...
### `[shell]`<VersionLabel version="0.56.0" />

#### `aliases`<VersionLabel version="0.56.0" />
...
#### `plugin`<VersionLabel version="0.60.0" />

The `plugin` field can be used to configure a
[plugin locator](../guides/wasm-plugins#configuring-plugin-locations) for the backend, as an
alternative to the [`[plugins.backends]`](#pluginsbackends) table. This allows the plugin and its
settings to be defined side-by-side.
...
The `plugin` field can be used to configure a
[plugin locator](../guides/wasm-plugins#configuring-plugin-locations) for the tool, as an
alternative to the [`[plugins.tools]`](#pluginstools) table. This allows the plugin and its
settings to be defined side-by-side.

```toml title=".prototools"
[tools.my-tool]
plugin = "https://raw.githubusercontent.com/my/tool/master/proto-plugin.toml"
```
