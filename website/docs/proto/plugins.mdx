...
plugin, and a WASM based plugin (coming soon!). Both of these plugins can be
[configured in `.prototools` and `~/.proto/config.toml` files](./config#plugins).
...
does not support any logic or complex behavior, and is merely for simple and common use cases, like
CLIs.
