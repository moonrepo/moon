...
```rust
#[plugin_fn]
pub fn register_metadata(Json(input): Json<ToolMetadataInput>) -> FnResult<Json<ToolMetadata>> {
  input.id
  // ...
}
```
...
_not_ support building from source. The `register_install_params` function _must_ be defined, whichs
...
```rust
#[plugin_fn]
pub fn register_install_params(Json(input): Json<EnvironmentInput>) -> FnResult<Json<InstallParams>> {
...
`~/.proto/bin/<id>`, but you can also register additional shims with the `register_shims` function.
...
pub fn register_shims(Json(input): Json<EnvironmentInput>) -> FnResult<Json<ShimParams>> {
    ...
    global_shims.insert("npx".into(), if input.os == "windows" {
        "npx.cmd".into()
    } else {
        "bin/npx".into()
    });
