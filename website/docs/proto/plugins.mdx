...
```rust
#[plugin_fn]
pub fn register_metadata(Json(input): Json<ToolMetadataInput>) -> FnResult<Json<ToolMetadataOutput>> {
  input.id
  // ...
}
```
...
_not_ support building from source. The `register_install` function _must_ be defined, whichs
...
```rust
#[plugin_fn]
pub fn register_install(Json(input): Json<InstallParamsInput>) -> FnResult<Json<InstallParamsOutput>> {
