...
The `npm` backend will install CLIs from the [npm registry](https://www.npmjs.com/) using the
`npm` package manager under the hood. The ID after the prefix is the package name as published to
the registry.
