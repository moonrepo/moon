...
When installed, proto will inject a `GOBIN` environment variable into your shell, pointing to
`~/go/bin`, which will be used to locate Go binaries.
...
When installed, the version of npm that came bundled with Node.js will also be installed.
