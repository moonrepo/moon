...
[context object](https://docs.rs/proto_pdk/latest/proto_pdk/struct.PluginContext.html):
...
- `working_dir` - A virtual path to the current working directory.
  <VersionLabel version="0.56.0" />
...
Functions that run _before_ a version has been resolved receive an
[unresolved context](https://docs.rs/proto_pdk/latest/proto_pdk/struct.PluginUnresolvedContext.html)
instead, which only provides the `proto_version`, `temp_dir`, and `working_dir` fields. Since v0.60,
the `tool_dir` and `version` fields are no longer available in this context.
...
- `checksum_public_key` - Public key used for verifying checksums. Only used for `.minisig`
  (minisign), and `.sig` / `.asc` (GPG) files. <VersionLabel version="0.61.0" />
- `post_script` - A script file, relative from the install directory, to execute after the prebuilt
  has been installed.
- `post_script_args` - A list of arguments to pass to the post-install script when executing it.
  <VersionLabel version="0.58.0" />
...
##### Verifying with GPG<VersionLabel version="0.61.0" />

Alongside hashes (`.sha256`, `.sha512`) and minisign signatures (`.minisig`), proto can verify a
download against a GPG detached signature. Point `checksum_url` at a `.sig` or `.asc` file, and
provide the project's armored public keyring through `checksum_public_key`.
...
- The keyring may contain multiple certificates within a single armored block, or multiple
  concatenated armored blocks.
- The signature file may contain multiple binary signatures, multiple signatures within one armored
  block, or multiple concatenated armored blocks. Verification succeeds when _any_ signature
  matches a trusted key.
- Only the verified signer fingerprint and a SHA-256 hash of the artifact are written to the
  [lockfile](./lockfile#checksum-verification), not the armored public keyring.

##### Unpacking an archive

Our plugin layer will do its best to detect file extensions, unpack the downloaded file (if an
archive), and install the tool to the correct directory. On macOS, `.pkg` installers and `.dmg` disk
images are also detected and unpacked automatically. <VersionLabel version="0.58.0" />

However, we're unable to account for _all_ edge cases, so for situations where the install params
above are not sufficient, you may define an `unpack_archive` function.
