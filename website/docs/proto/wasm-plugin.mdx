...
pub fn register_tool(_: ()) -> FnResult<Json<ToolMetadataOutput>> {
    Ok(Json(ToolMetadataOutput {
        config_schema: Some(schematic::SchemaBuilder::build_root::<NodeConfig>()),
        // ...
    }))
}
```
...
be achieved with the required `locate_executables` function. The `primary` field defines the
location of the executable, relative from the installation directory.
...
- [Official proto WASM plugins](https://github.com/moonrepo/tools)
