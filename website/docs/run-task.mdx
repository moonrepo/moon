...
with the [`moon run`](./commands/run) command. This command requires a single argument, a primary
[target](./concepts/target), which is the pairing of a project ID and task ID. In the example below,
our project is `app`, the task is `typecheck`, and the target is `app:typecheck`.
...
- Insert [`deps`](./config/project#deps) targets into the graph.
...
By default `moon run` will _always_ run the target, regardless if files have actually changed.
However, this is typically fine because of our [smart hashing & cache layer](./concepts/cache). With
that being said, if you'd like to _only_ run a target if files have changed, pass the `--affected`
flag.
...
> The `--` delimeter and any arguments _must_ be defined last on the command line.
